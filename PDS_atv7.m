...
Y_filtrado = filter(Hdveg1, Y); % obj Hdveg1
Y_filtradoTransform = abs(fft(Y_filtrado));
...
title('Stopband Filtered Signal')
subtitle('first stopband (object Hdveg1)')
xlabel('Frequency (Hz)')
ylabel('Magnitude')

% Recorded audio file filtered - 2
Y_filtrado2 = filter(Hdveg2, Y_filtrado); % obj Hdveg2
...
title('Stopband Filtered Signal')
subtitle('second stopband (object Hdveg2)')
xlabel('Frequency (Hz)')
ylabel('Magnitude')
